...
# 各施設に対応する device_id をマッピングする辞書 (仮に設定)
# device_id の数字部分と施設名を対応させる辞書
device_to_facility = {
...
campus_data["ni"] = campus_data.index.map(lambda name: facility_capacity.get(name, 100))  # niを設定
...
for device_id, people_inside in device_people_list:
    ...
    # 例: device_id が "1-people-count" なら "1" を取り出す
    ...
    if facility_name:
        ...
        campus_data.loc[facility_name, 'condition'] = people_inside / 100  # 仮に100人をキャパシティとする
